...
description: 创建、修改、优化、重组我们的可复用资产——技能(skill)、随技能的参考(reference，含该技能的事实规格+排错经验)、中心通用经验(playbook，与具体技能不强相关的运维/踩坑/妙招)，并维护它们与 AGENTS.md 的四层分层归置。触发场景：做个 skill / 写个技能 / 把这个流程沉淀下来 / 把这段经验记成知识 / 建个 reference 或 playbook / 改进某 skill 的触发词 / 优化更新某个技能 / 审阅别人加进来的内容是否合我们风格 / 把某段内容在 AGENTS.md·SKILL·reference·playbook 之间挪到该放的那层——任何把经验或流程固化成可复用资产的意图都用本技能。它先按四层归置原则判断内容落哪层(AGENTS.md 只留 playbooks 索引与技能原生发现说明、全局红线在仓库知识、SKILL 薄、随技能 reference 厚而全且自包含、playbook 只装与具体技能不强相关的通用经验且不随技能同步)，再判断做 skill / reference / playbook，然后复制 templates/ 现成骨架、按我们的目录命名约定产出或重组，落进中心目录 720_Agents。
---
...
> 🎯 **质量标杆 = `image-gen`**：薄 `SKILL.md`（只讲怎么把图生好）+ 随技能 `references/`（端点/字段/错误码等事实规格 **＋** 该技能的风控认知/调度哲学/踩坑排错——**与该技能强相关的一切都在这、自包含随技能走**）。建/改任何资产都**对照它的分层与写法**。
...
2. **复制模板改**：
   ...
   登记索引        → 仅 playbook：照 templates/agents-index-line.snippet.md 在 AGENTS.md 加一行（skill / reference 不登记）
   ```
3. 改完按 §「完成清单」逐项勾，确认无死链、（若建了 playbook）已在 AGENTS.md 登记。
...
|---|---|---|---|---|
| **① AGENTS.md** | 全局入口 | **Playbooks 索引** + 技能原生发现说明（全局红线已移仓库知识、不在此） | 隧道读中心库时 | `AGENTS.md` |
| **② SKILL.md** | 薄执行主干 | 触发/默认/命令/参数 + 指针 | 技能触发时 | `skills/<name>/SKILL.md` |
| **③ reference（随技能）** | **与该技能强相关的一切** | 该技能的事实规格（API 端点·参数·字段·错误码·凭证·阈值·schema）**＋** 该技能的排错经验/踩坑/认知 | 实现/排错时按需读 | `skills/<name>/references/<topic>.md` |
| **④ playbook（中心、独立）** | **与具体技能不强相关的通用经验** | 跨工具运维、日常通用错误、经验积累、秘诀妙招、SOP（如隧道 I/O、技能同步流程） | 需要时才整份取 | `playbooks/<name>.md` |

> 🚚 **谁随技能走、谁不走（跨账号同步的硬约束，创建时就得分清）**：② SKILL + ③ references **打包在 `skills/<name>/` 内，随 `.devin/skills/` 同步一起走**——所以**与某技能强相关的一切（事实＋排错经验）都进 ③ references**，技能自包含、能独立跑。④ 中心 playbook **不随技能同步**（跨账号/路 B 只带 `.devin/skills/`），平时不上传、需要时才取；所以它**只装与具体技能不强相关的通用经验**，技能正常运行绝不依赖它。

### ③ 与 ④ 的区别（分界 = 和某个技能强不强相关，务必分清）
- **reference（③）= 随技能走、和某个 skill 强相关的一切**：不管是"能查表的事实"（API 字段、错误码、凭证、schema、阈值）还是"该技能的排错经验/踩坑/认知"，**只要强绑定这个技能，统统放 `skills/<name>/references/`**。skill-native 工具（Codex/Claude）随技能自动加载，跨账号也随 `.devin/skills/` 同步走。判据：**这段东西是不是离开这个技能就没意义？** 是 → reference。
- **playbook（④）= 中心、独立、和具体技能不强相关的通用经验**：跨工具运维、日常通用错误、经验积累、秘诀妙招、SOP。**不绑任何技能、不随技能同步、平时不上传**，特地遇到才整份取给对方。判据：**这段东西换个技能/换个任务照样用得上吗？** 是 → playbook。
- 一句话准则：**强相关某技能（事实或经验都算）→ reference；不强相关的通用经验 → playbook。** 例：`image-gen` 的端点/字段/风控认知/排错心法都强相关 → 全进 `references/`；隧道 I/O 运维（`agent-ops.md`）、技能同步流程（`skills-sync.md`）谁都用得上、不绑技能 → playbook。

> ⚠️ **"reference" 一词只指 ③（随技能、强相关）**。中心 **playbook（④）** 不随任何 skill 打包，是跨任务、按需整份翻阅的通用经验容器——别把强相关某技能的东西丢这（那是 reference 的活）。

### 归置铁律
- **默认下沉**：拿不准一段内容放哪 → 往下放（reference 或 playbook 是兜底容器）；只有 happy-path 必需才上提 SKILL。**强相关某技能的（事实或经验）→ ③ references（随技能走、自包含）；不强相关的通用经验 → ④ playbook（不随技能同步、需要才取）。**
- **AGENTS.md 越瘦越值钱**：它常驻上下文，只收最重要的；可有可无、细节性的统统下沉。
- **唯一事实来源**：同一条知识只放一处，上层用指针引到下层，**绝不两边各抄一份**（会漂移）。
- **各层怎么被发现（决定要不要登记 AGENTS.md）**：**skill** 靠克隆进工作区的 `.devin/skills/` 被 Devin 自动扫描、被 Codex/Claude 按 `description` 触发；**reference** 随技能走、经该技能 `SKILL.md` 的指针读到——两者都**不登记 AGENTS.md**。**只有 playbook** 既不随技能同步、又不被原生发现 → **必须在 `AGENTS.md` 的 Playbooks 索引登记一行**。一句话：**建 skill / reference 不碰 AGENTS.md；建 playbook 才登记。**
...
| `agents-index-line.snippet.md` | AGENTS.md **Playbooks** 索引行格式 + 例子 | 建 playbook 时照着加/改一行（skill/reference 不登记） |
...
└── references/         （与该技能强相关的一切：事实规格＋排错经验，按需加载；多主题就拆多份）
```
> playbook **不在 skill 目录里**——它在中心 `playbooks/<name>.md`，装的是**与具体技能不强相关的通用经验**，由 AGENTS.md 索引串起。skill 目录只装"随技能走的"东西（SKILL + scripts + references）；**与该技能强相关的排错/认知也归 `references/`、不进 playbook**。跨账号同步只带 skill 目录、playbook 不跟着走，所以技能不能依赖 playbook 才能跑正常流程。
...
- 正文只留"**顺利把任务做好**"的执行主干（Quick Start/触发/默认/命令/参数 + 指针）。happy-path 不需要的厚重细节**下沉到随技能 `references/`**：事实规格、以及该技能的认知/根因/踩坑排错，**都进 `references/`**（随技能走、自包含）。SKILL 用指针指过去（**对照 `image-gen/SKILL.md` 顶部那段写法**）。只有与具体技能不强相关的通用经验才另立中心 `playbooks/`。
...
3. 这个 skill 没有 API 事实、也没踩坑经验 → 不建 reference；也没有不绑技能的通用经验 → 不建 playbook。
4. skill 靠 `.devin/skills/` 原生发现，**不用登记 AGENTS.md**（只有 playbook 才登记）。
...
2. **装与该技能强相关的一切**：事实规格（API 端点·参数·字段·返回、错误码清单、凭证名/header、数字阈值、机读 schema、退出码语义）**＋** 该技能的排错经验/踩坑/认知（为什么这么设计、撞过什么坑、怎么排）。**只要离开这个技能就没意义，就进来**；与具体技能不强相关的通用经验才归 playbook。
3. 内容多可拆多份（`references/<provider>-api.md` 放事实、`references/<name>-notes.md` 放该技能的排错经验），agent 只读相关那份；结构清晰：标题分节，编号用工具识别稳的形式（`3.1/3.2` 优于 `A/B/C`）；>300 行开头加目录。
4. **不登记 AGENTS.md**——reference 随 `.devin/skills/` 同步走、经该技能 `SKILL.md` 的指针读到；只需在 SKILL.md 里加/确认指向它的指针。
...
1. 在中心 `playbooks/` 下建 `<name>.md`，用**主题名**（如 `agent-ops.md` 隧道运维、`skills-sync.md` 同步流程）。**只放与具体技能不强相关的通用经验**；属某技能的深层经验/排错**不进这里**——并进该技能 `references/`。
2. **只装不绑技能的通用经验**：跨工具运维、日常通用错误、经验积累、秘诀妙招、SOP。**写厚、写全、经验拉满**——它是按需取用的容器，不随技能同步、平时不上传，需要时才整份给对方。
3. 结构清晰（标题分节、`3.1/3.2` 编号、>300 行加目录）。
4. **在 `AGENTS.md` 的 Playbooks 索引登记一行**（playbook 是唯一需要登记 AGENTS.md 的一层）。
5. 判据自查：**这段东西换个技能/换个任务还用得上吗？** 用不上（只对某技能有意义）→ 它不该在 playbook，挪进那个技能的 `references/`。
...
- **reference**：补全该技能的事实＋排错经验，保持可被工具稳定识别的结构；别混进与本技能不相关的通用经验（挪去 playbook）。
- **playbook**：尽管加厚通用经验；别混进只对某技能有意义的东西（挪去那个技能的 references）。
- 改完若动到触发/命令/参数，按 A5 跑 2–3 个真实 prompt 验证。

### D2. 审阅别人加进来的内容（合不合风格 + 该落哪层）
逐段过，对每段判它**该待在哪一层**：
- **该进仓库知识 / 该登记 AGENTS.md**：全局红线埋在某资产里 → 提进**仓库知识**（不是 AGENTS.md）；发现某 playbook 没在 AGENTS.md 登记 → 补一行（只有 playbook 需要登记，skill/reference 不登记）。
- **该留 SKILL**：happy-path 真需要且够重要的执行要点 → 留 SKILL。
- **该下沉 reference**：与某技能强相关的一切——事实规格（字段/错误码/schema/阈值）**＋** 该技能的根因/排错/踩坑 → 移到对应 `skills/<name>/references/`。
- **该下沉 playbook**：与具体技能不强相关的通用经验/运维/妙招 → 移到 `playbooks/<name>.md`。**拿不准是不是"通用"？默认归该技能的 references（随技能走更稳）。**
- **该删 / 合并**：和已有内容重复（违反唯一事实来源）→ 合并到唯一那处、删冗余；既不重要又冗长又没人会查 → 直接删。

### D3. 重组搬运的铁律
- **唯一事实来源**：搬走后原处只留指针，别两边各留一份。
- **搬动/增删 playbook 后同步 `AGENTS.md` 的 Playbooks 索引**那一行（路径/描述对得上；新建补行、删除删行，**别留死链**——曾经 `common-bugs.md` 索引指向不存在文件就是反例）。skill/reference 的增删改**不动 AGENTS.md**。
- 大改动前先跟用户对一下归置方案（哪些上提、哪些下沉、哪些删），再动手。

> 📌 **拆分实例（照着做）**：`image-gen` 的深层内容按主题拆成多份 `references/`——可查表的硬事实（端点/参数/错误码/凭证/manifest schema/阈值）→ `references/matsca-api.md`；该技能的认知/排错（风控根因/调度哲学/隧道脱离坑/排错心法）→ 同样归 `references/`（与该技能强相关、随技能走）。**两者都在技能内、自包含**；SKILL 用指针分别指过去——reference 随技能走、**不登记 AGENTS.md**。只有与具体技能不强相关的通用运维（如隧道 I/O）才另立中心 `playbooks/agent-ops.md`（**这个才在 AGENTS.md 登记**）。
...
- [ ] **技能自包含**：去掉中心 playbook 仍能跑通 happy-path（playbook 不随 `.devin/skills/` 同步走，别让正常流程依赖它）
- [ ] `python quick_validate.py <skill 目录>` 输出 "Skill is valid!"
- [ ] 跑过 2–3 个真实 prompt、给用户看过（若动了触发/命令/参数）

**建/改 reference 或 playbook：**
- [ ] reference 装与该技能强相关的一切（事实＋排错经验）、playbook 只装与具体技能不强相关的通用经验；两者不串味
- [ ] >300 行有目录；reference 多主题已拆份
- [ ] 引用对方内容时用指针、不复制（唯一事实来源）

**所有资产通用（收尾必查）：**
- [ ] `AGENTS.md` **Playbooks** 索引：仅当增删改的是 **playbook** 时加/改/删行（skill / reference 不登记 AGENTS.md）
- [ ] **无死链**：所有指针路径都解析得到真实文件
...
- **风格对齐 `image-gen`**：frontmatter 写法、中文正文 + pushy description、输出/目录约定、**薄 SKILL + 厚而自包含的 references（事实＋排错都在技能内）用指针衔接**——全部参照它。
- **跨工具通用**：skill/reference/playbook 是开放格式，Codex/Devin/Claude 都能用。本技能讲"产物长什么样、放哪、怎么登记"——和具体工具无关。
- **写入方式按工具不同**：Devin 经隧道改本地、只改不推 git；Codex 本地直接写中心目录。**具体 I/O 规则见仓库知识 / `playbooks/agent-ops.md`，本技能不重复。**
...
5. 按反馈改，重复到满意。
6. 按「完成清单」收尾：落中心目录、（仅 playbook）AGENTS.md 索引加/改/删、无死链、指针都解析得到。