...
> **事实型规格速查**——端点、参数、字段、错误码、凭证、在途数字、计费、机读产物 schema。能查表/照抄的硬事实都在这；**用法看 `SKILL.md`，根因/踩坑/排错心法看 `playbooks/image-gen-notes.md`。**
>
...
- ⛔ **`GET /v1/ping` 红线禁用**——即便官方文档说 ping 不耗积分，实测会触发风控封禁，绝不调用（根因见 `playbooks/image-gen-notes.md` §1）。
...
> 异步任务端点 `/api/image-tasks/*` 已不再使用（早期 direct 异步幂等方案已废弃，根因见 `playbooks/image-gen-notes.md` §3）。
>
...
## 6. 错误分类：可重试 / 不可重试清单
脚本已内置服务商官方的错误分类与退避（`with_retries`/`should_retry_error`/`retry_delay`），**不应手动重发**（为什么见 `playbooks/image-gen-notes.md` §2）。
...
- 该额度由"同一密钥下所有同时在跑的 `gen_image.py` 进程"**共享**（认知与防撞墙打法见 `playbooks/image-gen-notes.md` §4）。
...
## 13. 可观测性字段：.log / .heartbeat / status.py
（脱离/判活的操作心法见 `playbooks/image-gen-notes.md` §8，这里只列字段 schema。）
...
```
`atexit` 注册 `_final_heartbeat`：进程退出写一条 `alive:false, inflight:0` 终态。**`upstream_storm:true` = 上游网关 502 风暴**（应对 SOP 见 `playbooks/image-gen-notes.md` §7）。
