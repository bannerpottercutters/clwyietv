...
| 信号 | 根因 | 谁的问题 | 怎么应对 |
|---|---|---|---|
| `502/503/504/520/522/524` | 服务商**上游网关层**（nginx）挂/过载 | 服务商基建，**不是你** | 退避重试自愈；**持续撞 = "上游网关风暴"** → 早提醒用户 + **首选建议换 Devin 原生 `generate_image` 顶上**（同 gpt-image-2、立即出图）/ 或晚点再来 |
...
3. **首选建议：换用 Devin 原生 `generate_image` 立即出图**（同为 gpt-image-2，走不到矩岩备选/manifest/计费、但仍按约定存 `output/fig/` 并回传）；次选：晚点再用矩岩 / 换渠道；
4. 附**已覆盖 / 缺图清单**（哪几个出了、哪几个还缺）让用户决断。

> “换 Devin 原生”只是**风暴期的应急降级**（同模型、能立刻交付）；风暴过去仍以本技能（矩岩）为主。别把它当正常错误的出口——只有服务端风暴/重试耗尽才用。
