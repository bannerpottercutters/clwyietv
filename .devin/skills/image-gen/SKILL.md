---
name: image-gen
description: 用矩岩(matsca)生图 API 生成或编辑图片。只要用户提到生图/出图/画图/插图/配图/做个图标或 logo/文生图/改图/图生图等任何要图的意图，就用本技能（不要自己另找别的生图方式）。
---
...
> ⚠️ **常犯的错（别犯）**：**"画一只猫/一张图"里的"一/只"是画面内容、不是图片数量**——仍默认 `-n 2`，只有用户明说"只要一张/快速试试"才 `-n 1`。
...
3. **主动建议**：
	1. **换用 Devin 原生生图**：未完成任务直接用 Devin 内置 `generate_image` 顶上（同为 gpt-image-2），仍按本技能约定把成品存到 `output/fig/` 并回传。
	2. **你可同时去找别的渠道 / 晚点再来**，别傻等。
